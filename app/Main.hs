...
parms = defParams {closeDelay = 0.01}
delay = defParams {closeDelay = 1}
...
myMel1 = line [c 4 en, c 4 en, g 4 en, g 4 en, a 4 en, a 4 en, g 4 qn, f 4 en, f 4 en, e 4 en, e 4 en, d 4 en, d 4 en, c 4 qn]
...
myMel8 = Modify (Tempo 50.0) (myMel1 :+: myMel1 :+: myMel1 :+: myMel1)
myMel9 = foldr1 (:+:) $ fmap (Modify (Tempo 10000.0)) $ replicate 1000 myMel1
...
main = do
  ...
  -- mapM_ (\note -> playDev' 3 parms $ instrument AcousticGrandPiano $ note 4 qn) [a,b,c,d,e,f,g]
  mapM_ (\(note, octave) ->
           playDev' 3 parms $ instrument AcousticGrandPiano $ note octave qn)
    [(c,4),(d,4),(e,4),(f,4),(g,4),(a,4),(b,4),(c,5)]

  --sequence $ replicate 30 $ playDev' 3 delay $ (rest qn :: Music Pitch)    
  playDev' 3 delay $ (rest qn :: Music Pitch)
  ...
  playDev' 3 delay $ (rest qn :: Music Pitch)
  playDev' 3 parms $ chord $ map ($qn) [c 4, e 4, g 4, c 5]     
